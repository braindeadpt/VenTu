...
BEGIN
  ...
  IF NOT public.check_rate_limit(v_ip, 'submit_contribution', 5, interval '60 seconds') THEN
...
BEGIN
  ...
  -- ── Client IP + per-IP rate limit (shared helpers — supabase-rate-limit-common.sql) ──
  v_ip := public.request_client_ip();
  IF NOT public.check_rate_limit(v_ip, 'submit_score_feedback', 15, interval '60 seconds') THEN
    RETURN jsonb_build_object('ok', false, 'error', 'rate_limit');
  END IF;
