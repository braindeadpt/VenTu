...
test.describe('Map bathymetry (EMODnet WMS)', () => {
  test.use({ serviceWorkers: 'block', reducedMotion: 'reduce' });
  test.describe.configure({ timeout: 60_000 });

  test('toggle liga o overlay WMS, mostra a legenda e persiste a preferência', async ({ page }) => {
    await openMapa(page);

    // O atributo vive no wrapper do mapa (mesmo padrão de data-map-wind/cluster).
    ...
    await expect(map).toHaveAttribute('data-map-bathymetry', 'true', { timeout: 15_000 });
    await expect(toggle).toHaveAttribute('aria-pressed', 'true');
    // Pane dedicada + tiles WMS com o host EMODnet no src.
    await expect(
      page.locator('.leaflet-ventu-bathymetry-pane').first(),
