...
- **Ainda em baixo:** `tide_obs_nrt` (marés) — 500 desde 13/08 e, desde 29/07, o schema deixou de servir observações (items devolve 200 só-metadados, confirmado 2026-09-08). Camada `tideLayer` no pipeline-meta + chip no About + monitor com verificação de schema (nunca bloqueia o Open-Meteo); fallback EDR documentado (`IH_EDR_FALLBACK=1`) e OFF enquanto o radius devolver 500.
...
### 2026-07-24 — Locale FR (shell)

- `/fr/` + `translations.fr` + SEO; inline `isPt` ainda EN — trio ES/DE/FR completo (MVP)
