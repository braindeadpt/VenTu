...
| `spot-hero-cls` | 6 | Hero `#agora` (§1) com caixa reservada a 320/390/1350 px: altura constante do primeiro frame ao assentar (amostrador de 50 ms, CPU 4×) + nenhum layout-shift atribuído a uma caixa dentro do hero (com controlo que o tem de ver), nada cortado a 320 px e largura da coluna do nome independente dos dígitos do score (com controlo que repõe a grelha antiga e a tem de fazer falhar) |
...
Notas de operação:
- **Config** (`playwright.config.ts`): no CI usa `workers: 2` (runner 4 vCPU; browsers isolados por worker — medido ≈ 2m30s quando o core tinha ~115 testes) e `retries: 2` para flakes pontuais conhecidos (ex. `search palette` / sheet do mapa). Hoje o core são **311 testes em 47 ficheiros** (`npx playwright test <specs do core> --list`) — re-medir o tempo no CI se o passo apertar. O core completo (`npm run test:e2e:core`) corre como passo próprio no `ci.yml`.
