...
/**
 * Validates the source text of src/lib/spots.ts (regex-based, no transpile).
 * @param {string} content
 * @returns {{ errors: string[], spotCount: number }}
 */
function validateSpotsContent(content) {
  ...
  return { errors, spotCount: ids.length };
}

function main() {
  const content = fs.readFileSync(spotsPath, 'utf8');
  const { errors, spotCount } = validateSpotsContent(content);
  ...
  console.log(`✅ validate-spots: ${spotCount} spots OK — all have compatibleSports, ASCII-safe unique slugs`);
}

module.exports = { validateSpotsContent, SLUG_RE };
