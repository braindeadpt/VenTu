 ...
 * runs in any workflow, fails before build/E2E.
 *
...
const checkAscii = (label, list, sourceLabel) => {
  for (const s of list) {
    if (!SLUG_RE.test(s)) {
      errors.push(
...
const checkUnique = (label, list, sourceLabel) => {
  const seen = new Map();
  for (const s of list) {
    if (seen.has(s)) {
...
checkAscii('region slug', regionSlugs, 'seoLandings.ts REGION_SLUGS');
checkUnique('region slug', regionSlugs, 'seoLandings.ts REGION_SLUGS');

checkAscii('sport slug', sports, 'seoLandings.ts SEO_SPORTS');
checkUnique('sport slug', sports, 'seoLandings.ts SEO_SPORTS');
...
checkUnique('derived landing slug', composed, 'seoLandings.ts (sport + sport-region combos)');
...
checkAscii('modality slug', validSlugs, 'modalidades/[slug]/page.tsx VALID_SLUGS');
checkUnique('modality slug', validSlugs, 'modalidades/[slug]/page.tsx VALID_SLUGS');
...
console.log(`✅ validate-page-slugs: ${sports.length} sport slugs, ${regionSlugs.length} region slugs, ${composed.length} derived landings — ASCII-safe and unique; popular slugs and modality pages in sync`);