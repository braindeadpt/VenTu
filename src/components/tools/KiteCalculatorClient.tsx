    ...
    <div className="card-1 p-4 md:p-6 space-y-5">
      ...
      <div className="space-y-1.5">
        ...
        {spotId && !loading && pick?.windKt == null && (
          <p className="text-meta-sm text-score-fair">{t.tools.noWindData}</p>
        )}
      </div>

      <div className="space-y-1.5">
        <span className="text-meta-sm font-semibold text-fg-muted">{t.tools.discipline}</span>
      ...
      <div className="pt-4 border-t border-divider" aria-live="polite">
        {rec ? (
          <div className="space-y-3">
            <div className="flex items-baseline gap-3 flex-wrap">
              ...
              <span className="text-body-sm text-fg-muted">
                {t.tools.comfortWindow
                  .replace('{from}', String(rec.primaryWindow.fromKt))
                  .replace('{to}', String(rec.primaryWindow.toKt))}
              </span>
            </div>
          ...
          <p className="text-body-sm text-fg-muted">
            {t.tools.belowKiteMin.replace('{min}', String(KITE_WIND_KT_MIN))}
          </p>
        )}
        <p className="text-meta-sm text-fg-subtle mt-3">
          {t.tools.estimateNote}
        </p>
      </div>
    </div>
  );
